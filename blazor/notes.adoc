...
I already know about Postman, and the magic that it has with its collections.
I want to do something different, so I'll be using https://insomnia.rest/download[Insomnia].
Also, since I'm on Arch (btw), I will be installing it from the AUR (`yay -S insomnia-bin`),
just the bin, because the "build it yourself" version is actually broken.

Seems to start just like Postman, you get to login with many different options,
and there's a "guest" mode with apparent "limited capabilities".

We are going to launch the 4 different functions we have configured.

[NOTE]
====
So there's a cool Postman functionality that when creating a new collection/request
you can actually select a "Create API Basics" and it will create for you a collection
with 4 pre-loaded requests (CRUD).
====

We can then get based on our created Realtime Database, a URL to hit, we are going
to consume a `server` resource. Always suffix the whole URL with `.json` that's a
requirement. Otherwise you will get hit with a login screen or some error message.

````
<FIREBASE-URL>/servers.json
````

We can then hit a POST:
...
And then the API should respond with the unique key for the record:

````
{
	"name": "-Ob-NVPdCgJT50AwgTIN"
}
````

The Unique Key will always be auto-generated and be treated as the "root" of the
document. This is how `POST` works in Firebase, if we don't like that, we have to
leverage `PUT`. That way we can send the body to something like `/servers/1.json`,
and we will be able to insert the record with the `id` we want.

_NOTE:_ This is not a standard case, it's how we have to go around Firebase's
conventions. In an ideal world, we would have to use `POST`.

_NOTE 2:_ Probably something that has to do with the fact we are forcing Firebase
to work with our own **Ids**. When we list servers, we will see one `null` entry.
And the course suggests we write "code around that", bad, bad.

We can then play around the path, the verb, and do all the CRUD operations that
we know. (If you know, you know)

==== Invoke API in C#

The normal process to start talking Restful, is to serialize all the content we
want to send to an API (into JSON). Attach that as a Body and send it over the
network.

The Web API will process the Request and it will come with a response (THAT WE HAVE
TO DESERIALIZE).

So now, we'll start sending requests from our Application.

Luckily MS already has built-in things for us, we just have to install `Microsoft.Extensions.Http`
on Main and Client projects of the WASM Blazor app.

As you can notice, a good pattern and practice is to always leverage the DI Container,
that way we inject managed services in our components that we leverage as easily
as adding one line of code. The pattern of adding them through `Program.cs` also
allows for us for easier config, debug and extension.

The course _hard codes_ the firebase URL. That's so bad. I'm adding an `appsettings.Development.json`
into the client project to see if this will work. Also adding that same file name
to `.gitignore` so that we don't commit to version control.

[IMPORTANT]
====
Yeah... WASM in Blazor is just not good, not a really good implementation, it's a
hassle to work with, replicating code, doing really annoying things. So choose
InteractiveServer, that's a better approach.
====

I say this, because we can't leverage configuration values, to hide, and **NOT
HARDCODE THE URL AS THE COURSE SUGGESTS**.

In short what took place:

. We created an IApiRepository, ApiRepository. And we then copied the code twice
for Client and Main project for the DI Injection as `Transient`

We then injected an Http client:

````
builder.Services.AddHttpClient("ServersApi", client =>
{
    client.BaseAddress = new Uri("<HARD-CODED-URI>");
    client.DefaultRequestHeaders.Add("Accept", "application/json");
});
````
It's a good practice to give it a name, so that we retrieve a client with a specific
set of configurations. _However, this should be externalized, no magic strings_.

And lastly on the page for `Servers.razor` we injected the repo through its **interface**.
That's it. Of course, we are leveraging async methods now since HttpClient by default
adheres to this **BEST PRACTICE**. And we have to modify code so that it talks
async, instead of sync. (So bubbling async-await wherever it is needed).

Okay, one interesting find is that at the http client level we can literally use
methods that allow us to skip over installing newtownsoft or manually deserializing
and stuffs:

````
var response = await client.GetFromJsonAsync<List<Server>>("/servers.json");

var response = await clientGetAsync("/servers.json");
response.EnsureSuccesStatusCode();

return response.Content.ReadFromJsonAsync<T>();
````
So if we leverage `GetFromJsonAsync` or `GetAsync` or `ReadFromJsonAsync` we can
avoid installing Newtownsoft, or manually deserializing contents.

[NOTE]
====
Another thing I didn't like is the fact that the course writes implementation and
leverages the "Extract Interface" Refactor always. Better to design from abstraction
level first than start coding directly. That way you manually think about the things
you will need and then start implementing one by one.
====

==== Prepare Body in HttpRequest

We know this. We will now leverage code to add a body to a POST:
...
----
<.> This isn't production ready. We are making a second call through the network
and to the database just to get one scalar. Not optimal
<.> Leveraging this method to auto-serialize, and abstract things that aren't that
important.
<.> Used a local function since it seems only this method will leverage it, putting
a `return` before the declaration of the local function is a good way to separate
the runtime and also logically.
<.> This way we can wrap the result and act on it.
<.> The compiler itself says it's impossible, it's not, because of the `null` that
we mentioned when listing, this is also an alarm though, we might be using the
API wrong.
<.> When cast into a list, we should leverage `Count` instead of `Any`, since one
is a literal property access instead of some processing method that enumerates.
<.> This also is not that performant as far as I'm aware, we will be reading again
the whole collection just to get the Max `ServerId`, but whatever.

_Note:_ Another reason I hate we are hardcoding the URL of the database. This could
send a wrong message to Juniors. They might think it's okay to commit that. IT'S
NOT, IT'S UNSECURE AND YOUR VERSION CONTROL TURNS AGAINST YOU RIGHT THERE.

==== Implement the Api Repository

Not much to say here. It's pretty standard:

. Add Update, List By Id methods to the ApiRepository
. Consume them from the app component's, and then async all the way up since that's
what the syntax involves
. Be careful of the _delete URL_ I coded a delete of the whole database instead
of one record
. Also, when you delete, send that instruction and retrieve the fresh list of record,
you need to do a `StateHasChanged()` right after otherwise you won't get the screen
updated with the new records.
. Ran into the "OnParametersSetAsync" issue that it will try to make a request
**twice** on the `EditServer.razor` component. Moved away from that method and
went into "OnAfterReder"'s pattern. With this we are only making one request
guaranteed, and it still works the same.'

=== Authentication & Authorization

==== Overview - How authentication works in Blazor

Authentication working in daily life:

- Say you go to a theme park.
- When you want to play inside the theme park, you have to purchase a ticket.
    - This has to be printed, plastic card. Either way you have to purchase it.
    - Once you present the ticket, and you have to get on a ride, the worker's will
    ask you for your ticket. You need to present that ticket all the way to be allowed
    to get on. But perhaps your ticket only has access to certain rides. This "action"
    that restricts an already provided ticket is called **AUTHORIZATION**.
        - Not everyone is allowed, there may be constraints, some might not get
        a ticket. This is **AUTHENTICATION**.
- How does this work in Blazor?
    - In Blazor in .NET is a bit complex.
    - On traditional web apps. You would authenticate with a server, get a cookie
    back, store the cookie in the browser, and any other subsequent request should
    have to go back to the server with the cookie attached, the server would then
    validate the cookie, and if it's all good it would run the code you are telling
    it to run. This is a **stateless** type of authentication. Because the server
    doesn't keep track of the "tickets" of the users.

We need to work around the three types of Routing we discussed before:

- Static Routing = This adheres to the traditional web app stateless pattern.
- Server Interactive Routing = (Location is Global). In this case there's a
SignalR channel between the client and server. Cookie **DOES NOT LIVE AT THE
BROWSER SIDE**, it will be stored at the SignalR channel itself as **authentication
state**
- Web Assembly Interactive Routing = The whole app is downloaded to the browser,
your authentication ticket is stored within the application as _persistent state_.
Within the WASM application, **not the browser's local storage**.

==== All authentication scenarios

In Blazor within .NET 8 is a bit complicated, hence we will drill down bit by bit
every scenario for authentication/authorization.
...
- Static Routing
    - Blazor SSR
    - Server Interactivity with per component/page interactive location
    - WASM Interactivity with per component/page interactive location
- Server Interactive Location
    - Server Interactivity with a Global interactive location
- WASM Interactive Location
    - WASM Interactivity with a Global interactive location

We will go through each of these scenarios one by one in order to really understand
and breakdown how Authentication and Authorization are broken down depending
on scenario.

==== Authentication in Blazor SSR

We will create a new project. No interactive mode and Auth = Individual Account.

Just by setting these options for scaffolding we get components ready for the whole
Auth flow in mind.

- It even comes with an already built-in migration.
- It comes with `Identity` already implemented into the DI container
- It even comes with EF Core already added and integrated as well, alongside an
`ApplicationDbContext`

So `Identity` is a "feature", pretty mature actually, that's already part of
AspNet.Core.

If you go and look at the `appsettings.json` file, it will even have a connection
string already configured to try and work with SQLite (local, lightweight database
that's saved in a file(s)), we don't want to use that though. Let's integrate with
the SQL Server database that we already have running on Docker:

````
"DefaultConnection": "Data Source=localhost;Initial Catalog=AspNetAccounts;User ID=sa;Password=Str0ngPassw0rd@;Trust Server Certificate=true"
````

[NOTE]
====
And now, let's just say that there's _course magic involved here_. Because in the
video, it literally shows changing this connection string to a SQL Server string and
_it just works_. But in fact, it doesn't _just work_. We have to make some changes
====

. Install Microsoft.EntityFrameworkCore.SqlServer
. Switch the SQLite DI Container injection statement and replace with SQLServer's
+
````
builder.Services
...
````
. Delete the `.db` and all migration data that was scaffolded (this is all SQLite
focused).
. dotnet ef migrations add Init --project ./Security.BlazorSSR.csproj --startup-project ./Security.BlazorSSR.csproj
. dotnet ef database update

And after all of this, we should have everything running correctly, and we can
connect to our SQL Server's `ApsNetAccounts` database and check the structure
that was generated.

If you run the application you will get a screen with 3 navbar items that we didn't see
previously:
...
[NOTE]
====
Dev password is `abc123@Z`
====

If we go into `Register` we can try to register a new user with a password.

At this point in time you will get a "dev debug screen" stating that the app doesn't
have an actual email integration configured to it. And so, it will be necessary
to press on the link there to **auto-confirm** the email. In a normal flow a user's
email should get a confirmation email that he can click to then confirm the registration
to the website.

After we press the "auto-confirm" page, we can skip over that and stick to development.

_NOTE:_ This is all AspNet.Core's Identity Feature, a mature and complex integration
that helps us setting up Authentication and Authorization out the box without having
to build anything from zero. It's complex, so it can be its own course on its own,
right now _we will be just scratching the surface_.

After this we can login with the user we created, and the third navbar item would
switch to "Logout".

If we logout and open up the DevTools, and head to the "Cookies" section under our
apps domain. We should actually see tons of cookies, (The course only shows one,
but I'm guessing that newer versions of Identity add more metadata). This default
cookie is the Antiforgery cookie. However, once you login, you will get a new cookie
under `.AspNetCore.Identity.Application`. This cookie is the session one.

If you head to any page e.g., `/weather` or the navbar page `Auth Required` and
look at the `Network` under the DevTools, you will see there's always a header
`Cookie`, the course shows a structure that on newer .NET versions is no longer
applicable so we will skip over it, but it just states that it sends both an anti
forgery cookie concatenated with the actual user cookie through a semi-colon.

In our case it's a fully encrypted string, oh yeah, even if you want to look into
the cookie on the browser, it's fully encrypted, it's ***secure.***

==== User Registration

We will deep dive a bit on how the user registration works.

The page that does the registration is at `Register.razor`.

If you have a look at the Form's `OnValidSubmit` you will see how it creates a
`User` instance based on a custom type called `ApplicationUser` right now its code
is empty, but this pattern is a good practice with extensibility in mind:
...
So as you can see, to tie this into an `Identity` user, you would just inherit
from `IdentityUser` and add extra functionality/data as you see fit on this
specific inherit class.

If you actually start reading the code:
...
----
<.> Check how it makes these calls in sequence, one to set the user's username.
<.> And another call to set the user's email. And with this code you might immediately
notice that both username and email are the same.
<.> This is a really important piece of code. This takes care of taking the user
**in-memory** loaded instance, and send the password that's in the form and try
to save it to the database. The password _of course_ is saved as a HASH, not as
plain text. This `UserManager` class is really something you will use a lot and
it's dependency injected.
<.> Lastly, if the registration of a user is succesful, we will attempt to send
the confirmation e-mail and re-direct the user to another page.

==== Login process

You can analyze `Login.razor`, it basically is a form that captures 3 pieces of
data:

. Username
. Password
. If you want to persist the session or not

`LoginUser()` is the bulk of the logic.
...
----
<.> This method takes care of trying to log-in, to save the cookie to the browser
and then enable the app to start leveraging said cookie. How does the app know
about the user's info? Because inside the cookie there's an authentication ticket,
and said ticket contains the user's info/metadata.

And then based on different failure conditions then the app can react for 2FA, if
the user is locked out, if the login has failed, or if succesful we just go to the
next screen.

_NOTE:_ And for security purposes if a failed attempt is recognized we have to display
a **really generic error message**.

==== Authorization process

Under the premise that we have gotten our cookie back from the server. The moment
we want to access a different page, that's when the server will ask for a cookie,
and validate it. This process is the **Authorization**, this makes sure a user is
logged in, and that the user has the right level of access to the resource.

If we head to the `Auth.razor` component, we can easily notice the `@attribute [Authorize]`
attribute.

Any page/component that you want to mark to validate for access needs this small
attribute added to it. We can put to test this idea, by simply taking this decorator
and putting it in the `Weather.razor` component.

[NOTE]
====
A good idea would be to put under `_Imports` the `Microsoft.AspNetCore.Authorization`,
namespace, since this would widely be used, and we should really try and get our
component code to be as clean as possible
====

If we logout, and try to head down to the `Weather` menu item, we would actually
be redirected to the login page automatically. If you take a look at the URL
`/Account/Login?ReturnUrl=%2Fweather` you will notice how it actually holds a
route parameter, after successfully logging in we would then be redirected to the
previously visited page that redirected us. (This is a really nice UX, and I think
we should inspire ourselves in such an interesting design pattern)

If we delete manually the `AspNetCore.Application` cookie, and refresh the page,
we would again get redirected to the login page. This corroborates the idea that
this cookie is extremely important, and it really holds the ticket for a user in
order to be allowed to get in or get out. That's the underlying mechanism.

[IMPORTANT]
====
This is simply a check for "IS HE LOGGED IN"? Nothing else, permissions are a
different topic that we will cover later. But just mark this as important, that
protecting your page/component with `Identity` can be as simple as that.
====

==== Accessing the authentication ticket

Authorization is based on the information that's inside the authentication ticket
that lives in the cookie.

Once we are authorized to see the page, we make some logical decisions based on
particular data that the user holds. How can we access this metadata?

````
Hello @context.User.Identity?.Name!
````

This line of code is really important in `Auth.razor`. That whole `@context.User...`
is the way to access user metadata. The `context` property is actually a class
of type `AuthenticationState`, and it lives in the underlying base component.
From there we can acess this `User` property that's of `ClaimsPrincipal` type.
And by drilling down we can then get access to user metadata, such as in this
instance, the `Name`.

_Normally, what's on the authentication ticket?_
...
Any personal information attached to a user is recorded as a key-value pair.

That's what's on a regular ticket, you can keep this ticket and go to the authorized
places. (In the places they can then double check your personal data for further
logic-checks. E.g., You should be age 50 in order to ride a rollercoaster, or should
be under 180 cm).

This idea of leveraging metadata in order to enhance further granular checks, is
also applied in **Identity**. Now if we try to leverage the `Identity` property
we saw previously, we wouldn't get that much information to be honest. For accessing
to more metadata (and also custom metadata) we can leverage `Claims`:

````
@context.User.Claims
````

The `Claims` property is a key-value data structure (Dictionary). We can debug a
bit about how the claims here look like with this code:
...
// And this is something we can get printed
http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier: 3a105806-923e-4446-a192-f77e8da773b6
http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name: diego@diego.com
http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress: diego@diego.com
AspNet.Identity.SecurityStamp: 4T3PWYRQNQAMQL5PTDMMBBWOIOIE3JYV
amr: pwd
````

There are default claims with metadata and things that we might not have direct use
for but that the Framework requires, on top of other things that are much more
humanly understandable such as `name`, `emailaddress`. And as mentioned before,
_custom properties can be added_.
 
If you wanted to add more claims, you can insert into the `AspNetUserClaims` table
more records, this is a table that literally mirrors the key-value pair pattern.

[source, sql]
----
INSERT INTO AspNetUserClaims <.>
VALUES ('3a105806-923e-4446-a192-f77e8da773b6', 'Role', 'Manager') <.> <.>
----
<.> And so a simple insert into the table should suffice.
<.> The UserId can be retrieved from the `nameidentifier` claim.
<.> And in here a small detail, if we have "identity" keys, or keys that are
auto-generated, we don't have to insert a `null`, but we can simply skip over
that column as if it didn't exist, and we can start putting in the other values
we want for the record.

This is an example of an inserted record on top of another claim.:

1,3a105806-923e-4446-a192-f77e8da773b6,Role,Manager
3a105806-923e-4446-a192-f77e8da773b6,Department,Software Development

If we were to reload the Blazor app, and try to search for the new claims we just
inserted we would easily notice how these new claims don't show up. _WHY?_ Because
the claims info is stored within the cookie, we already logged in, and got a cookie
at a point in time in which these claims didn't exist, and so our state is technically
"inconsistent". So by re-login, we actually get to see new claims.

And now, by accessing these claims and values, we get access to control authorization
at the app level with this authorization info.

Other names: _Security Context_.

==== Authentication in Server Interactivity per page component

We will create a new Blazor application for security and with ServerInteractivity
per page.

Once the app is created, we will now just follow a bit of the previous steps we had
to install and connect to SQL Server, and we won't rely on Migrations anymore, since
we have a database now, we will hot-wire to the already created database.

[NOTE]
====
I guess this is another "good point" you can make for going for the full Microsoft
ecosystem driven approach. And that is that if you follow standard structures and
procedures then you would be able to seamlessly change, switch between different
apps/clients and they would all still be speaking the same language.
====

After we have run all the installation steps, we should be able to run this new
project, and even login with the previously registered user we used on the SSR
project.

We can apply the same experiment of logging in, Dev Tools, Delete `.Application`
cookie, and then reload the page. We would get kicked out just the same.

What is happening in this setup:

. We are still using static routing
. We are still sending HTTP Requests to the server in order to get back a new
page (you can easily corroborate this by looking at the Network tab of Dev Tools)
. Interactivity is applied only at the page level, so that we get reactive interactions
without JS.
. At the Identity level, everything still works the same

==== Authentication in WebAssembly Interactivity per page component

Just create a new project but in this case WebAssembly Interactivity and per-page,
that's it. (And run the same Sql connection setup so that we can talk to the database)

You will notice something specific. The interactive pages are `Counter.razor` and
`Auth.razor`. They are at the `.Client` project. And they have set for them
`@rendermode InteractiveWebAssembly`.

**NOTE:** In order to install SqlServer and establish the connection you need to
install it at the _Main_ project. After you set everything up there, then you
should be able to connect to the SqlServer database without issue.

. Because the interactivity location is per-page/component, the routing is still
static. (Only the components are downloaded to the browser). So there will be
Http requests made to the server, we still server pages as in the SSR and Server
Interactivity server. (WE ALSO SEND COOKIE OVER).

Even though this is a Blazor WebAssembly project, and because they use `cookie`,
we use the Authorization ticket, it's the same. _Don't worry about it_.

[NOTE]
====
Sometimes Rider doesn't create projects on the first try. Don't really know why.
Just try again, _don't give up_
====

==== Authentication in Global Server Interactivity

- New Project
    -- But this time a Global Interactivity Location
- Connect to SQL

So some details here:

. If you go into `App.razor` you will notice how we have a specific attribute
applied to the `<Routes>` component.
...
<head>
    ...
    <link rel="stylesheet" href="@Assets["lib/bootstrap/dist/css/bootstrap.min.css"]"/>
    <link rel="stylesheet" href="@Assets["app.css"]"/>
    <link rel="stylesheet" href="@Assets["Security.BlazorServerInteractivityGlobal.styles.css"]"/>
    <ImportMap/>
    <link rel="icon" type="image/png" href="favicon.png"/>
    <HeadOutlet @rendermode="PageRenderMode"/>
</head>
...
----
<.> As you can see it's at the router level that we apply `@rendermode`.
<.> And unless we override this setting at a component level, then by default we
will be working with interactive server routing.

The course shows that the scaffolded project, for whatever reason makes the `/Account`
path be an exception to this. So `Indetity` works with static routing for whatever
limitation/reason. But on the new .NET versions, this pattern seems to have **changed**.

And this new pattern is really interesting,

[source, csharp]
.Components/Account/Pages/_Imports.razor
====
@using Security.BlazorServerInteractivityGlobal.Components.Account.Shared
@attribute [ExcludeFromInteractiveRouting] <.>
====
<.> At the root of the `Pages` folder we have this `_Imports.razor` which immediately
makes me draw parallels to what typescript has with its `barrels` feature. But in here
this is acting as a barrier to apply globally to all pages under `Account` (this
is just an assumption), and so any `/Account` page/component will not be treated
as an interactive routing component.

- If we start the app and head to Home, we will see the initial handshake request
- If we head to `Counter` no network request will be logged anymore, and that is
because the SignalR leveraging kicks in
- If we head to `Weather` no network request is loaded either
- If we head to any of the auth pages however, a request to the server will be
made and we will get a network log. We get kicked out, redirected and the page
reloads.
- **BUT**, if we actually Login, and we now try to navigate to any page, THEN WE
WILL HAVE SIGNALR FULLY. We won't get any more Network logs. **THIS IS A REALLY
IMPORTANT CONCEPT YOU HAVE TO KNOW IN ORDER TO DEBUG CORRECTLY).
- What's even more interesting, is that if you go to the `Storage > Cookies`, and
you delete the `AspNetCore.Application` cookie, and try to head to the pages that
require Auth, you won't get kicked out, the app would still recognize you as
"he's still logged in". _Why?_ Well, we already mentioned this, because the SignalR
channel is the one maintaining the cookie now, it's no longer the browser acting
as the source of truth.
    - If you want to get kicked out (lol), just do a hard refresh of the page.
    The moment we refresh the page, the SignalR channel is broken and a new
    one is established, and this new instance, of course, has no memory of a previous
    channel nor its state.

_NOTE_: Once we are in, then the interactive location becomes server interactive,
hence the browser cookie is no longer used for further exchanges. This is the
same flow, but small under the hood details vary slightly, nothing more. _It simply
works differently, that underlying mechanism is different_. But you use things
the same.

[IMPORANT]
====
So in short, you will leverage the SignalR channel to get the authentication ticket,
not the browser after a succesful login.
====

==== Authentication in Blazor Global WebAssembly Interactivity

Everything we've seen so far is applied with a slight change, looking at the main
project's `App.razor`:
...
<head>
    ...
    <link rel="stylesheet" href="@Assets["lib/bootstrap/dist/css/bootstrap.min.css"]"/>
    <link rel="stylesheet" href="@Assets["app.css"]"/>
    <link rel="stylesheet" href="@Assets["Security.BlazorWebAssemblyGlobal.styles.css"]"/>
    <ImportMap/>
    <link rel="icon" type="image/png" href="favicon.png"/>
    <HeadOutlet @rendermode="PageRenderMode"/>
</head>
...
----
<.> As you can see, the only change here is that by default all components will
run with `InteractiveWebAssembly`.

Yet, when looking at the `_Imports` from the Account folder, we would see how it
excludes all subordinate components/pages there from this default setting.

_Note:_ This is a really interesting pattern that I think I could also inspire myself
in for different types of problems. It's better than the other approach the course
shows and seems to be the initial approach the Blazor team took but later refactored
and made way more decoupled and extendable.

- The same idea is applied still. All pages are SignalR leveraged, if you aren't
logged in, the log in pages are Static Routing. Once you login however, interactive
rendering kicks in.

[IMPORTANT]
====
Okay, I think I made a mistake. In the sense that after login everything will
be web assembly and interactive rendering. But if you head to the `Manage Account`
page, this will be treated as Static Routing. **ALWAYS**
====

Now the course explains something that is no longer applied in .NET 9. So first let's
go through the course's explanation to understand concepts first, but later we will
reconcile how this is no longer applicable/should be adapted to the new Framework's
conventions.

_COURSE_

- If we head down to the `Client` project:
    - `PersistentStateAuthenticationProvider.cs` is a class that is here declared.
    This class that lives at the client level, will try to retrieve persisten info
    that's on the **client**
    - And it is actually used in the `Program.cs` by adding it as a `Singleton`
- If we head down to the main/server project:
    - We will have code that provides the persisting authentication state data
    to the client, _because this info has to come from somewhere_.
    - `PersistingServerAuthenticationStateProvider.cs`
        - This has a `OnPersistingAsync` method that takes care of persisting the
        `userId` and `email`.
        - The course goes into manually adding user info (claim info) on the
        server side persistance class. (E.g., Adding a `Role` property)

[NOTE]
====
Oh no.............................. The scaffold has weird behaviors and errors.
Microsoft WHY? If you head to the Auth Page, you will see for like 2 seconds the
claims we used to see on our previous test when adding custom claims to a user.
And then it will refresh and only show **one claim**. And if you try to logout
when standing on this page, we will get hit with an exception.
====

The course into the fact that in order to add custom claims and stuffs and for that
to be exposed and seen, you have to manually go and add code that will encapsulate
that data that's on the database and that Identity also exposes.

_NET 9 Approach_

Instead of having all these extra boiler-plate classes, by default serialization
is handled through an extension method on the server-main side:

````
// New in .NET 9 - handles serialization automatically
builder.Services.AddAuthenticationStateSerialization();
````

You can however, extend this stock-option easily through inheritance:
...
And then on the **Client** side, you also just call an extension method:

````
builder.Services.AddAuthorizationCore();

// New in .NET 9 - deserializes authentication state automatically
builder.Services.AddAuthenticationStateDeserialization();
````

But notice how the server is in charge of **Serialization**, whilst the Client
works with the `Deserialization`.

Now, let's stop here for a second to think:

_Why did Microsoft change the pattern?_ It can never be "because it's cooler this
way". There must have been an issue, or a chance to make things better, to make it
"easier". So we should aim at leveraging that new approache's "better" pattern.
And so one alternative and recommendation is instead to leverage this whole custom
mapping to a component that renders really Early.

_NOTE:_ The Main Project references Client, not the other way around.

- And the flow for persistance has to be:
    - Server persists the data (serializes it).
    - The Client retrieves the persistent data and deserializes it.

[NOTE]
====
So yeah, WASM was a great concept, idea. But for production and just ease of
work, this "method": on Blazor is not mature enough, it's cumbersome, bothersome
overall. Stick to server side rendering (funny how the React community would hear
this and go crazy). Server Interactivity is **KING**
====

==== Secure our server side Blazor app

Let's try to apply Authentication and Authorization to our existing project.

This exercise is mainly to show how can you add `Identity` into an existing
project that didn't have it before:

. From a scaffolded Auth project we had, copy over the whole `Account` folder
(Under Components) to the same path but in the target project
. Copy over the `Data` contents to its target project counterpart
. Add the `Microsoft.AspNetCore.Diagnostics.EntityFrameworkCore` Nuget
. Add the `Microsoft.AspNetCore.Identity.EntityFrameworkCore` Nuget
. After that, you will have to adjust all namespaces, imports and stuffs (without
Rider this would be such a hassle, I Know)
. And then the `Program.cs` needs to have all the DI container services injected
on top of extra configurations so that all the workflows work correctly
. There's also a Diagnostics package line: `app.UseMigrationsEndPoint();` that
should also be copied over.
. Don't forget to pass over the connection string with a name that doesn't clash
with the one we have (`appsettings.json`)
. We have to also change the `Routes.razor` component to use a different _Route View_
on top of coding the action in case the user is not authorized.
. And lastly the `NavMenu.razor` component should have added under an `<AuthorizedView>`
component all the code when it comes to the Authorization endpoints and stuffs. We
also need to pass over all the `@code` code, and inject the `NavigationManager`
. Don't forget to put as much as you can both in `GlobalUsings.cs` and `_Imports.razor`

_NOTE:_ Looking at the `AuthorizedView` usage on this scaffolded items you still
can get the sense of how it's used so you then use it. If you want to wall something
with auth, you should wrap them with `AuthorizedView` and you can also configure
what to show in case the user isn't authorized.

A good practice will be to now run the app and check if we can login with the previously
used user (which in turn lets us now we are connecting to the database correctly),
and of course, that we are actually building the project without errors.

With all of this setup. We should now think in terms of the actual app. _What do
we wanna protect?_

We will do:

- The Add Server Page
- The Edit Server Page

As it was mentioned before there's a `@attribute [Autorize]` that we can leverage
to inject into our components to protect them.

With just that, we can head into the servers page, try to Edit a server, and then
(whilst not logged in) we would get redirected to the Login page, once we login
we would get redirected back to the screen we were denied entry from.

[NOTE]
====
This idea of leveraging route parameters to save the pages that triggered a request
for auth, and once authorized throwing back the user to his previous workflow is such
a nice pattern (UX wise), I will try to leverage it as much as possible.
====

_NOTE:_ I noticed in the console logs an exception is thrown, it apparently doesn't
affect the user experience, but remember, _no broken windows_ on **THIS HOUSE**,
so be sure to research that exception some time.

==== Policy based authorization

Let's now expand further what we did, besides a simple "Yes-No" type of check,
let's make a more granular check, i.e., If the user is a "Manager" _then_ he can
edit servers. Normal users can add, edition is manager only.

**Policy based authorization:** Leveraging `Identity` we can apply this feature so
that we reach our desired result. We have to configure the authorization.

In the `Program.cs` you would add something like this:
...
````
This structure is pretty simple, the `AddAuthorization` extension method allows us
to start adding the controls we want, through policies. In this instance we are adding
a policy called `Manager`, and the logical check for it is to ensure that the logged
in user has a `Claim` under a `Role` key and with a `Manager` role. You can add more
values since this second argument is of `params[]` type.

How did we get this info? By looking at the `AspNetUserClaims` table of the Identity
database, in it we have our previously created user with this claim.

And if you want more policies you can simply start adding more lines just like this
one.

_Extra Note:_ A more recommended approach is to use a newly introduced builder
instead:
...
````
This is less verbose, but also, by reading on the docs, it integrates better with
new report, exception and login APIs.

Once all of this is done, the way to leverage this policy and enforce it is by
adding something extra to the previously added attribute in a component:

````
@attribute [Authorize(Policy = "Manager")]
````

See the relationships here, we added a policy with a `Manager` name, and in here
we are enforcing the usage of a **Policy** of the same name. This is how things
connect, _don't forget that_.

If we go into the app and create a new user, say `tester10@tester.com` and check on
the database the claims table, we will immediately notice how no info in regards
to this new user is present. Hence he doesn't have the `Manager` role. With this
very same user we can try and go into the add/edit server pages, and we would get
redirected to the login immediately.

_The course also did the whole wizard setup that I didn't code, and it adds a layer
of specificity by having this new user have access to the Add - Wizard _flow_, but
when trying to edit this being solely for managers. The idea, and implementation
is interesting, so at least _keep that in mind_.

Lastly, if we want for this user to actually have the right to edit, we would then
insert the claims at the database level for his ID. Now, this is ugly of course,
we would never want to do this in prod, a management interface of any kind (doesn't
necessarily have to be UI) would take care of adding these claims and stuffs with
Identity's API (because it has endpoints)
...
This is a by default turned-on feature in Blazor, yet it has some consequences.
It is available for Server interactivity and Web Assembly interactivity.

Server Interactivity:

Whenever we are doing the initial handshake between browser and server, the user
_without pre-render_ would need to wait for the server to get a request, process
it, send it back to browser, browser interpreting all of the content and then
getting a screen. It's a long process, that's _less responsive_ because it all
hinges on the fact that the SignalR channel needs to be established.

Besides the less-response issue, we also have issues with web crawlers (search engine
indexing). A robot will start scanning a website, and because things would take
time to render, the crawler would interpret as the whole website being completely
empty. Which would then rank the website pretty low. It would rank the static pages
maybe, others not. For SEO purposes no pre-rendering is baaad.

WebAssembly Interactivity: Yes, this is more _responsive_ in general, but it runs into
the same issues as Server Interactivity, SEO will be affected since a pre-ramup period
takes place when talking to a server, getting the necessary files and data and then
starting the WebAssembly portion of the app. The first response will be WebAssembly
code, no usable content on the web page.

So _pre-rendering_ takes care of getting a request, processing everything as fast as
possible, instantiating all necessary components in memory, getting everything ready
to the point of HTML and sending that back to the client.

_HTML reaches the web page_, so it can display in the DOM automatically. With that
_pre-baked_ HTML the blazor js file will come, and he will do the whole handshake on
its own. The moment the SignalR channel is established then that pre-render component
is completely disposed of, and the server tries to rebuild said component now
with the handshake between client and server finished.

So what's the "problem" with pre-rendering:

- We will instantiate components **twice**. So most things would be called twice,
if you have `expensive` component initialization code, then all that logic will
be run twice more than likely.

_NOTE:_ WebAssembly also gets an ad-hoc in-memory component representation from the
very beginning. But the moment a handshake is established, and we "cut" the server
connection, or rather we confirm that we provided the WASM app with all it needs,
then this component gets disposed of. And the WASM whole runtime and processes
kick in.

So there's a need for pre-rendering and the problems it introduces. Now we will
talk about the different solutions we can apply to leverage the good from both
worlds.

==== Solution 1: Use OnAfterRender

So, pre-rendering has problems. Main Problem: Component Initialization can run
many times.

First solution to it: `OnAfterRender`.

We have been doing this already, if we head down to the `ServerListComponent.razor`
code, we will see how we are indeed leveraging the `OnAfterRender` hook, and at the
same time we control that this logic only runs once.
...
````
However, this approach _apparently is less responsive_. This loads initially in the
server, it's kinda bad but still nice since we haven't disabled pre-rendering, all
the code that runs statically, it still is statically rendered, the user doesn't
look at a blank page.

[NOTE]
====
Yep, I should have followed the "Smart-dumb" component approach, I have now run
into an issue here. The course will run a solution that relies on the list of
servers being at the `Servers.razor` page, luckily this code can still be put
in the children component, but this indeed can turn into something ugly the more
we try to extend.
====

So, to improve the experience a bit, we can run something like this on our server
list component:
...
----
<.> Notice how we are applying the return early principle here. Otherwise we will
go and render the actual list content.

We can then artificially mock a slow server response by doing an `await Task.Delay()`.
And we will easily see how much of a nice experience it is to see a message instead
of letting the user sit and wait for a blank screen.

**Second Problem:** This is bad for SEO still, _why?_ Because SEO will only see
the response after _n seconds_. The SEO will then deem the site as really slow on
performance and rank it low. It's better to pre-render your data, but it's more
of a suggestion.

[IMPORTANT]
====
Be sure to leverage `OnAfterRender` and its `firstRender` flag to solve the problem
of multiple initialization code runs. This works for Server and WASM interactivity.
You can solve "slowness" by adding conditional renderings of loading messages/components.
SEO is affected in a way, although not in a _"horrible"_ way.
====

==== Solution 2: Disable Pre-Rendering

Multiple component initialization can also be solved by simply turnin pre-rendering
off. Makes sense, doesn't it? This whole problem is starting **because** pre-render
is turned on.

The way to disable pre-render we would write something like this:

````
// This is locally, so just a component can have it disabled, (as an attribute)
rendermode="new InteractiveServerRenderMode(prerender: false)"

// This is explicit render syntax. (Should be at the top of the component)
@rendermode @(new InteractiveServerRenderMode(prerender: false)

````

If you remember ways back how we logged component hook execution, we used to see
two sets of events, they sort of "repeat" executions (or that's what it seems like).

With this turning off of prerender we can see how those "repeated" executions do not
in fact log twice now.

[NOTE]
====
The course showed how to _locally_ turn off pre-render. This however is nullified
immediately if said component is a Child of another component that has pre-render
still on. The parent would re-render twice, and the child in turn would also be
submitted to do the same. So if you want to really stop double inits, be sure to
track down the component tree and disable it at the root of everything. Still,
if you are using a component in `Blazor SSR` or something specific, you can definitely
disable prerender at a local level as it has been laid out in the code excerpt
up above.
====

If we were to do this in WASM, we can log at logging, and we will something really
interesting. The Log rendering is split between Server - Client. One set of hooks
would run on the server (and we can see those logs in the console), and the other
set of hooks would run on the Client (and we would see console logs in the Dev
Tools).

_The moment we turn prerender off_ we will stop seeing the Server logs. And only
the console logs show up.

[IMPORTANT]
====
Still, this is **LESS RESPONSIVE**. Everything has to be downloaded before code
is executed. So it takes a while to see results, it's not good on SEO at all.
Before loading, screens are blank and we can't be ranked good.
====

Still, this can solve our problem, things will look very smooth, and we would get
rid of the double execution thing.

_Internal Tools:_ Nobody cares. We can make a choice to just turn it off. Buddy.

==== Solution 3: Use PersistentComponentState

There's some docs you can read from Microsoft for this, https://learn.microsoft.com/en-us/aspnet/core/blazor/components/prerender?view=aspnetcore-9.0#persist-prerendered-state[LINK].

In here we have code examples and theory, which is nice, but in summary:
...
@code {
    ...
    private async Task PopulateCitiesAsync()
    {
        if (!string.IsNullOrWhiteSpace(SearchCity))
        {
            servers = await ServersRepository.SearchServersAsync(SearchCity!);
            return;
        }

        servers = await ServersRepository.GetServersByCityAsync(CityName ?? FALLBACK_CITY);
        StateHasChanged();
    }
...
====
<.> We should inject this specific `PersistentComponentState`. We will leverage
it in its entirety.
<.> And we will store in a state variable a subscription which will interface with
the `ComponentState`.
<.> And so on `OnInitializedAsync` we will make a check against the state service first,
under one specific **key** we will try to retrieve data. If the service returns something
to us, we happy and we can assing said (_typed_) restored data to our local state
variable without having to call expensive logic.
<.> Because I abstracted the whole logic in its own method, this piece of code
looks way cleaner than the course variation.
<.> This is from the docs, the course does something different and in my end the
app broke. By the end of everything, that's when we call this method that binds to
a `Task` method in order to try and save state into the store. This spits back a
subscription that we keep stored.
<.> We still need this piece of code so that we re run our logic when parameters change,
(this feeds into the workflow that after you press a city, it will filter servers of
that server). Without this piece of code, then that flow would break.
<.> And this is the method that was mentioned some points above, this has to be
sent to the method the state service exposes, this is a callback that internally
will be called to run and somehow register state persistence.

So this approach leverages `OnInitializedAsync` and it will run twice as we know,
because it is `InteractiveServer` and because we have `prerender` set to **true**.

- We still have pre-render
- We avoid calling expensive logic (init logic)
- We leverage a framework exposed service (the App State) to act as a cache
- We move away from `OnAfterRender` since using this hook is a bit counter-intuitive,
we actually focus on `OnInitialized` a hook than in its name should scream it being
where all our init logic should be.
- But _most importantly:_ Our flow **still behaves the same, but way more optimized**.

This is the actual recommended approach. (It still adds some overhead and boiler-plate
code).

The code in the end does say that it would be in all Blazor developers hopes that
Microsoft implements a much smoother, cleaner way of going around these desings
considerations. But for now, the recommended approach would be this specific
